...
- **Security** — новый пароль, путь админки, ограничения доступа по домену/IP и ключ Management API.
...
**Management API key** на вкладке **Security** включает `api/manage.php`. Пустое поле выключает API. **Generate** создаёт новый ключ, **Copy** копирует его. Сохранение применяет значение. Запросы без этого ключа возвращают `404 Not Found`, пока не включён Debug mode. Ключ кампании PHP Connect и `pbkey` постбэка — отдельные секреты.
